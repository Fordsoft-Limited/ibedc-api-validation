...
import pandas as pd
import json 
from app.custom_app_error import InvalidDataFormatException
    ...
    def _parse_constraints(self, constraints):
        ...
        if isinstance(constraints, str) and constraints.strip():
            try:
                ...
                raise serializers.ValidationError("Invalid JSON format for constraints.")
        else:
            constraints = {}  # If empty or not provided, return an empty dict
        if not isinstance(constraints, dict):
            raise serializers.ValidationError("Constraints must be a valid JSON object.")
        return constraints
